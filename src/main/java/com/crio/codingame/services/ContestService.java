...
public class ContestService implements IContestService {
    ...
    private List<Question> pickQuestionsList(final List<Question> questions,final Integer numQuestion){
        List<Question> curatedList = new ArrayList<>();
        List<Question> questionsCopy = new ArrayList<>(questions); // To ensure that a random question is not picked twice
        Random random = new Random();
        for (int i = 0; i < numQuestion; i++){
            int randomIndex = random.nextInt(questionsCopy.size());
            curatedList.add(questionsCopy.get(randomIndex));
