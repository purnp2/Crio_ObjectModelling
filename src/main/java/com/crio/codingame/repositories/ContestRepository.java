...
public class ContestRepository implements IContestRepository {
    ...
    public boolean existsById(String id) {
        ...
        if (contestMap.keySet().contains(id)) return true;
        return false;
    }
    ...
    public long count() {
        // TODO Auto-generated method stub
        return contestMap.keySet().stream().count();
    }
    ...
    public List<Contest> findAllContestLevelWise(Level level) {
     return contestMap.values().stream().filter(p -> p.getLevel()==level).collect(Collectors.toList());
    }
