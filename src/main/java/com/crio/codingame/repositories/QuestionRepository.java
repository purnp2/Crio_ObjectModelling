...
public class QuestionRepository implements IQuestionRepository {
    ...
    public boolean existsById(String id) {
        ...
        if (questionMap.containsKey(id)) return true;
        return false;
    }
    ...
    public long count() {
        // TODO Auto-generated method stub
        return questionMap.keySet().stream().count();
    }
    ...
    public List<Question> findAllQuestionLevelWise(Level level) {
        return questionMap.values().stream().filter(p -> p.getLevel() == level).collect(Collectors.toList());
    }
